...
当前已分发候选：`v0.13.4-internal`，数据版本 **schema 15**，`UNSIGNED-INTERNAL` 团队测试 / Pre-release。只生成 Windows 10/11 x64 与 macOS Apple Silicon arm64 两个原生安装包。

本次内测候选：`v0.13.4-internal`，按同名标签构建两平台内测安装包，**已于 2026-09-21 真机验收通过后分发**。本手册描述的运行入口对 0.13.2 同样有效；0.13.4 在它之上增加了 0.13.3 的审计修复与本轮出差路线页的收口，还没升级的成员看到的仍是 0.13.2 的界面。

覆盖升级：任何 schema 写入前自动生成并验证本机 `pre_upgrade` 备份，再迁移到 schema 15；从 0.13.1 覆盖升级不触发迁移。真机新装、覆盖升级与重启保数已通过；第二台设备激活仍 Pending，代码签名与公证未做，因此不标记为 Stable / Latest。
