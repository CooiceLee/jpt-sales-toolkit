...
**已分发的 `v0.13.4-internal`**：真实 Windows 与 Apple Silicon 设备上的新装、覆盖升级、重启保数已于 2026-09-21 通过（[docs/v0.13.4-validation-result.md](v0.13.4-validation-result.md)）。仍标 **Pending** 的是**第二台设备的正式激活**——同一个人换一台电脑，走完 `.jptreq` → `.jptauth` → 验证码 → 登录。谁先遇到这种情况，请把结果反馈回来。

**这一版仍未签名**：Windows SmartScreen 与 macOS Gatekeeper 会拦一次，需要手动确认；代码签名与 Apple 公证都还没做，所以它不是"公开正式版"。

**标 〔候选〕 的部分**：这些能力在 `v0.13.4-internal` 里，还留在 `v0.13.2-internal` 上的成员看不到，升级后即可照做。日常使用中发现与本文描述不符的地方，直接反馈。
