...
| | 版本 | 内容 | 状态 |
|---|---|---|---|
| **已分发给团队** | `v0.13.2-internal` | `v0.13.1-internal` 的全部功能，加一处页面路由越权读取的安全修复 | 未签名内测 Draft Pre-release。真实 Windows / Apple Silicon 设备的新装、覆盖升级、重启保数仍为 **Pending**，见 [docs/v0.13.2-validation-result.md](v0.13.2-validation-result.md) |
| **本次内测候选** | `v0.13.4-internal` | 0.13.2 的全部内容，加两轮修补：手工录入与客户匹配（同名公司都能选到）、列表完整分页、面板保存状态、出差四分区、金额按币种、翻译不再改写业务文本、读不到设备标识时在启动页直说；再加本轮的出差路线页收口——一条时间线读全程、交通段按真实日期与距离分行、日期导航与地图定位、「设置约定时间」入口，以及保存边界（一次保存只提交它自己的字段，拜访目的被别人改过时会先问你保留哪一份） | **按 tag 构建两平台内测包，尚未分发**。真实设备的新装、覆盖升级、重启保数与第二台设备激活仍为 **Pending**，见 [docs/v0.13.4-validation-result.md](v0.13.4-validation-result.md) |
| **本文标 〔候选〕 的部分** | 即 `v0.13.4-internal` | 上一行列出的那些能力 | 拿到 0.13.4 安装包之后才能照做；你现在手上的 0.13.2 里**没有** |

版本号在程序启动日志 `data/logs/launcher.log` 第一行。带 〔候选〕 标记的段落要装上 `v0.13.4-internal` 之后才能照做；未标记的段落在 `v0.13.2-internal` 上就能走完。
