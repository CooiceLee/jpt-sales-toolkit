...
| **本次内测候选** | `v0.13.3-internal` | 0.13.2 的全部内容，加本轮的修补：手工录入与客户匹配（同名公司都能选到）、列表完整分页、面板保存状态、出差四分区、金额按币种、翻译不再改写业务文本，以及读不到设备标识时在启动页直说 | **已构建两平台内测包，尚未分发**。真实设备的新装、覆盖升级、重启保数与第二台设备激活仍为 **Pending**，见 [docs/v0.13.3-validation-result.md](v0.13.3-validation-result.md) |
| **本文标 〔候选〕 的部分** | 即 `v0.13.3-internal` | 上一行列出的那些能力 | 拿到 0.13.3 安装包之后才能照做；你现在手上的 0.13.2 里**没有** |

版本号在程序启动日志 `data/logs/launcher.log` 第一行。带 〔候选〕 标记的段落要装上 `v0.13.3-internal` 之后才能照做；未标记的段落在 `v0.13.2-internal` 上就能走完。
...
| 〔候选〕导出行程时提示**"行程已过期，请重新预览并保存。"** | 改过停靠或成员之后日程需要重排 | 回到行程页重新预览并保存，再导出 |
